import type { Metadata } from "next";
import { HH_BASE_METADATA } from "@/data/site-content";
import { ALWAYS_NOINDEX_ROBOTS } from "@/lib/seo";
import { HHShell } from "@/components/hh/HHShell";
import { CheckoutContent } from "@/components/hh/checkout/CheckoutContent";

// P2.9: checkout route stays noindex regardless of NEXT_PUBLIC_SITE_ENV.
export const metadata: Metadata = { ...HH_BASE_METADATA, title: "Thanh toán", robots: ALWAYS_NOINDEX_ROBOTS };

export default function ThanhToanPage() {
  return (
    <HHShell>
      <CheckoutContent />
    </HHShell>
  );
}
